...
namespace RUtil.Graphics
{
    ...
    public static class ColorUtil
    {
        ...
        static internal class KnownColorTable
        {
            ...
            public static Color32 KnownColorToColor32(KnownColor color)
            {
                var argb = KnownColorToArgb(color);
                byte a = (byte)((argb >> ARGBAlphaShift) & 0xff);
                byte r = (byte)((argb >> ARGBRedShift) & 0xff);
                byte g = (byte)((argb >> ARGBGreenShift) & 0xff);
                byte b = (byte)((argb >> ARGBBlueShift) & 0xff);
        ...
        /// <summary>
        /// スラッシュ区切り。"1/1/1/1"でwhiteなど。#rrggbbaaや色名にも対応
        /// </summary>
        ...
        public static Color Parse(string str)
        {
            ...
            {
                throw new Exception("Parse error: 無効な文字列 [" + str + "]");
            }
        }
        ...
        public static bool TryParse(string str, out Color result)
        {
            ...
            if (ColorUtility.TryParseHtmlString(str_, out result))
            {
                return true;
            }

            if (TryParseFromHtml(str_, out var systemCol))
            {
                result = (Color)systemCol;
                return true;
            }

            if (str_.ToLower() == "clear")
            {
                result = Color.clear;
                return true;
            }
            ...
            try
            {
                ...
                if (args.Length == 4)
                {
                    ...
                    result = new Color(c[0], c[1], c[2], c[3]);
                    return true;
                }
                else if (args.Length == 3)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                        c[i] = float.Parse(args[i]);
                    result = new Color(c[0], c[1], c[2], 1);
                    return true;
                }
                else
                {
        ...
        public static bool TryParseFromHtml(string htmlColor, out Color32 result)
        {
            // empty color
                ...
                ((htmlColor.Length == 7) || (htmlColor.Length == 4)))
            {
                ...
                {
                    result = new Color32(Convert.ToByte(htmlColor.Substring(1, 2), 16),
                                  Convert.ToByte(htmlColor.Substring(3, 2), 16),
                                  Convert.ToByte(htmlColor.Substring(5, 2), 16), 255);
                }
                else
                {
                    ...
                    result = new Color32(Convert.ToByte(r + r, 16),
                                    Convert.ToByte(g + g, 16),
                                    Convert.ToByte(b + b, 16), 255);
                }
                return true;
            }
            ...
            {
                result = KnownColorTable.KnownColorToColor32(KnownColor.LightGray);
                return true;
            }

            if (Enum.TryParse(htmlColor, true, out KnownColor known))
            {
                result = KnownColorTable.KnownColorToColor32(known);
                return true;
            }
            result = default;
            return false;
        ...
        public static Color Jet(float value, float min, float max)
        {
        ...
        public static Color Jet(float value)
        {
            if (float.IsNaN(value))
            {
                return Color.black;
            ...
            return new Color(
                Mathf.Clamp01(1.5f - 4f * Mathf.Abs(value - 0.75f)),
                Mathf.Clamp01(1.5f - 4f * Mathf.Abs(value - 0.50f)),
                Mathf.Clamp01(1.5f - 4f * Mathf.Abs(value - 0.25f))
                );
        }
        ...
        public static Color Lab(float l, float a, float b)
        {
            ...
            var y = l <= 8 ? (l * 100) / 903.3f : 100 * Mathf.Pow((l + 16) / 116, 3);
            var y2 = l <= 8 ? 7.787f * (y / 100) + 16 / 116 : RUtil.Mathematics.MathUtil.Cbrt(y / 100);
            var x = a / 500.0 + y2 <= 0.2069f ? 95.047f * (a / 500 + y2 - 16 / 116) / 7.787f : 95.047f * Mathf.Pow(a / 500 + y2, 3);
            var z = y2 - b / 200.0 <= 0.2059f ? 108.883f * (y2 - b / 200 - 16 / 116) / 7.787f : 108.883f * Mathf.Pow(y2 - b / 200, 3);

            var (x1, y1, z1) = (x / 100, y / 100, z / 100);
            var cr = (x1 * 3.2406f) + (y1 * -1.5372f) + (z1 * -0.4986f);
            var cg = (x1 * -0.9689f) + (y1 * 1.8758f) + (z1 * 0.0415f);
            var cb = (x1 * 0.0557f) + (y1 * -0.2040f) + (z1 * 1.0570f);
            cr = Mathf.Clamp01(cr > 0.0031308f ? (1.055f * Mathf.Pow(cr, 1.0f / 2.4f)) - 0.055f : cr * 12.92f);
            cg = Mathf.Clamp01(cg > 0.0031308f ? (1.055f * Mathf.Pow(cg, 1.0f / 2.4f)) - 0.055f : cg * 12.92f);
            cb = Mathf.Clamp01(cb > 0.0031308f ? (1.055f * Mathf.Pow(cb, 1.0f / 2.4f)) - 0.055f : cb * 12.92f);
